...
/// Generic SDP manipulation for WebRTC sessions: filters codec choice and
/// injects bandwidth hints. Protocol-agnostic — this is plain SDP text
/// rewriting, not specific to any one cloud-gaming backend, so it should work
/// as-is once we know which codecs Boosteroid actually offers.
///
/// TODO(protocol): confirm which codecs Boosteroid's server offers in its SDP
/// offer (H.264 is the safe assumption; H.265/AV1 support is unconfirmed) and
/// whether it needs the same H.265 tier/level safety rewrites GFN's servers do.
enum SDPMunger {
